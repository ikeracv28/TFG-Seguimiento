...
class MensajeControllerTest {
    ...
    private MensajeResponse mensajeMock() {
        return new MensajeResponse(1L, 1L, 2L, "Alumno", "Ejemplo",
                "Hola desde el historial", LocalDateTime.now());
    }

    @Test
    @DisplayName("ALUMNO puede obtener el historial de mensajes de su práctica")
    @WithMockUser(roles = "ALUMNO")
    void alumno_puede_ver_historial() throws Exception {
        when(mensajeService.listarPorPractica(1L)).thenReturn(List.of(mensajeMock()));
    ...
    @Test
    @DisplayName("TUTOR_CENTRO puede obtener el historial de mensajes")
    ...
    @Test
    @DisplayName("TUTOR_EMPRESA puede obtener el historial de mensajes")
    ...
    void historial_vacio_devuelve_array_vacio() throws Exception {
        when(mensajeService.listarPorPractica(99L)).thenReturn(List.of());

        mockMvc.perform(get("/api/v1/mensajes/practica/99"))
                .andExpect(status().isOk())
